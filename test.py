...
class wypok:
    def __init__(self):
        self.api = wykop.WykopAPI("0BMyXOrKfm","gwtxRIaie1")
        self._auth()
                ...
                for key in get_from_wypok:
                    if key.new ==True:
                        if key.type == "entry_comment_directed":
                            ...
                            Notifier.notify(key.author+"\ndodal komentarz.", title="Wykop.pl",appIcon="images.jpg",sound="default",open=key.url)
                        elif key.type=="pm":
    ...
    def _auth(self):
        self.api.authenticate("pawisoon", "wOWldJLvlhhnTXqIW8Zc")
